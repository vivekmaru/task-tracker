...
func writeProposedActionForm(w io.Writer, ticketID pgtype.UUID, action string, label string, placeholder string) {
	...
	if action == "reject" {
		class = ` class="destructive"`
		confirmation = ` onsubmit="return confirm('Reject this proposed work? It will be removed from the triage queue.');"`
		required = ` required`
	} else if action == "archive" {
		class = ` class="destructive"`
		confirmation = ` onsubmit="return confirm('Archive this proposed work? It will be removed from the triage queue.');"`
		required = ` required`
	}
